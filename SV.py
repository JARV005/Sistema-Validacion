def Proceso_Compra():
    ...
    for i in range(len(Productos)):
        ...
        precio_sin_descuento = Precios[i] * Cantidades[i]  # Costo sin descuento
        total_producto = precio_con_descuento * Cantidades[i]
        total_compra += total_producto
        print(f"\nProducto: {Productos[i]}")
        print(f"Precio unitario: {Precios[i]:.2f}")
        print(f"Precio total sin descuento: {precio_sin_descuento: }")
        print(f"Descuento aplicado: {PorcentajeDes[i]:.2f}%")
        print(f"Cantidad adquirida: {Cantidades[i]}")
        print(f"Precio final por producto: {precio_con_descuento: }")
        print(f"Total por {Productos[i]}: {total_producto: }")

    # Mostrar el total de la compra
    print(f"\nEl costo total de la compra es: {total_compra: }")
